...
현재 Phase 0~11-0의 문서·설계 gate는 완료됐다. Phase 11-0에서 Python runtime, `uv`, 최소 dependency, quality command와 Phase 11-1 파일 범위를 확정했지만 실제 generator, validator, CLI, Web UI 또는 installer 구현은 시작하지 않았다.
...
Fragment·skill·profile의 draft lifecycle과 `reviewed-for-mvp`, `ready-candidate` 파생 판정은 `catalog-readiness-policy.md`에서 설명합니다. 이 정책은 catalog YAML status를 변경하지 않습니다.

Phase 11-1의 runtime/dependency, module 범위와 acceptance gate는 `mvp-file-structure-plan.md`, `mvp-implementation-entry-plan.md`, `mvp-module-boundary.md`, `mvp-acceptance-criteria.md`에서 설명합니다. 초기화와 구현 파일은 Phase 11-1 승인 전까지 생성하지 않습니다.
