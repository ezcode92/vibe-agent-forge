...
- 현재 MVP 영향: 기존 6개 profile과 Codex dry-run은 이 relation을 선택하지 않으므로 영향이 없다. Catalog에 존재한다는 이유만으로 전체 실행을 차단하지 않는다. 향후 resolved 입력이 이 relation을 선택하면 error/blocked다.
...
- 현재 MVP 영향: 기존 preview와 dry-run은 이 relation을 선택하지 않으므로 readiness 영향이 없다. 선택 입력에 포함되면 지원 완료로 처리하지 않고 error/blocked로 보고한다.
...
- MSA 대상 generator validation과 representative dry-run 범위가 별도로 승인된다.

조건이 충족되기 전에는 두 relation과 registry YAML을 그대로 유지한다.

## MVP 판정 요약

- Unselected pending: `ignored`; 전체 catalog load와 지원 조합의 readiness를 막지 않는다.
- Selected pending: `error`; preview status는 `blocked`다.
- 현재 `ready` dry-run 6종: 두 pending relation을 선택하지 않으므로 영향 없음.
