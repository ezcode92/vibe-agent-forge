...
- Catalog lifecycle status: Registry YAML에 기록된 자산 자체의 수명 주기다. 현재 fragment, skill과 profile은 `draft`다.
- Dry-run readiness: 특정 profile·variant·adapter 조합의 수동 검증 결과다. `ready`, `ready-with-warnings`, `blocked`를 사용한다.
- Generator validation status: 한 요청의 check 결과와 preview 가능 상태다. Error/warning/info 및 readiness로 표현한다.
- Implementation readiness: 실제 generator, validator, writer 또는 adapter code를 구현·배포할 준비 상태다. Catalog나 dry-run 상태에서 자동으로 파생하지 않는다.

한 축의 상태가 다른 축을 자동 변경하지 않는다. `draft` 자산을 사용한 dry-run이 `ready`일 수 있으며, 이는 해당 조합이 검토됐다는 뜻이지 catalog status가 `ready`로 승격됐거나 구현이 완료됐다는 뜻이 아니다.

## Draft 자산의 MVP 해석

- Fragment, skill과 profile의 `draft`는 단독으로 warning이나 error가 아니다.
- 선택 조합이 Codex `ready` dry-run의 profile, variant와 resolved source 집합에 정확히 포함되면 generator는 `reviewed-for-mvp` 파생 판정을 info로 기록할 수 있다.
- 구조·참조 검증은 통과하지만 정확히 대응하는 `ready` dry-run이 없으면 `ready-candidate` 파생 판정과 warning을 기록한다. Error가 없으면 `ready-with-warnings`는 가능하다.
- `reviewed-for-mvp`와 `ready-candidate`는 registry status 값이 아니며 YAML에 기록하지 않는다.
- Missing path, dependency, bridge, variant 또는 conflict는 `draft` 때문이 아니라 해당 검증 실패 자체로 error다.
- Codex adapter의 `mvp-contract`만 MVP target으로 허용한다. Claude/Gemini의 `draft` adapter 선택은 unsupported target error다.
...
- 선택 profile이 pending relation을 포함하면 error로 기록하고 preview를 차단한다.
- 선택 조합에 포함되지 않은 pending relation은 해당 실행의 validation 대상이 아니며 readiness를 낮추지 않는다. Catalog inventory 정보로만 조회할 수 있다.
