...
- Profile `output.size_budget`을 적용하되 현재 문자열 정책은 warning 기준으로 수동 해석한다.
- 길이 초과 시 중복 설명, background, optional 절차 순으로 분리 후보를 제안한다.
- 필수 규칙을 유지하고도 budget을 만족하지 못하면 절단하지 않고 error 또는 수동 검토로 처리한다.
