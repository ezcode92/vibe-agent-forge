...
현재 Spring, React와 Flutter fragment 3개가 존재하며 관련 bridge, database/API/architecture fragment, skill, profile과 registry에 연결된다. 이 문서는 framework/platform 책임을 정의하며 generator와 adapter 실행 기능은 구현하지 않는다.
