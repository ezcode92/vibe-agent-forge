...
`templates/`는 VibeAgentForge가 관리할 생성 단위의 표준 구조를 정의한다. 템플릿은 실제 구성 자산이 아니라 작성자가 책임, 입력, 검증과 참조 관계를 빠뜨리지 않도록 하는 에이전트 중립 초안이다.
...
YAML 템플릿의 placeholder는 문자열로 인용되어 있으므로 초안 상태에서도 구조를 읽을 수 있다. 실제 항목에 해당 값이 없다면 placeholder를 남기지 말고 빈 목록 또는 합의된 생략 규칙을 사용한다. 구체적인 필수·선택 필드와 schema 검증은 실제 catalog 계약을 확정할 때 정의한다.
