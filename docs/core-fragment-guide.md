...
현재 core fragment 3개와 기술별 fragment, bridge, skill, profile YAML 및 registry catalog가 문서 자산으로 존재한다. 이 문서는 core의 조합 책임만 정의하며 generator, installer, adapter 실행 기능과 Web UI는 구현하지 않는다.
