...
Frontend와 mobile의 component, rendering, navigation, platform 연동 skill은 다음 단계에서 작성한다. 이번 분류에 임시로 포함하거나 backend workflow를 재사용한다고 가정하지 않는다.
...
현재는 backend 4종, database 2종, testing 1종과 automation 1종의 `SKILL.md` 초안만 정의한다. Frontend/mobile skill, product-specific skill, profile YAML, skill catalog와 실행 구현은 아직 만들지 않는다.
