...
| Template contract wording consistency | Template이 `mvp-contract` 호환 preview/export 구조임을 밝히고 미확정 상태 표현을 사용하지 않음 | Warning |
...
- Template placeholder 자체는 입력 slot이므로 허용한다. 다만 template 안내 문구가 확정 전 상태를 나타내거나 실제 project instruction·file write 완료로 오인될 수 있으면 warning이다.
