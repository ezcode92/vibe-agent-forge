...
현재 RDB·NoSQL, RESTful API와 CLI Automation을 포함한 6개 architecture fragment가 존재하며 bridge, skill, profile과 registry에 연결된다. Product-specific fragment, generator와 실행 기능은 현재 범위에 포함하지 않는다.
