...
Codex 대상 대표 조합으로 backend, Python CLI automation, React frontend, Flutter app과 fullstack의 Kotlin/Java variant를 검증한다.
...
| `fullstack-spring-react` Kotlin | `examples/codex/fullstack-spring-react/` | `ready` | Kotlin–Spring bridge와 fullstack API 경계 |
| `fullstack-spring-react` Java | `examples/codex/fullstack-spring-react-java/` | `ready` | Java–Spring bridge와 fullstack API 경계 |

각 위치의 README, preview, merge trace, skill selection과 validation report는 수동 검증 example이며 실제 export artifact가 아니다.

`fullstack-spring-react`의 Kotlin과 Java option은 각각 대응 Spring bridge를 선택한 별도 dry-run으로 검증했다. 두 option을 동시에 병합하지 않으며 각 결과는 같은 profile의 대체 preview example이다.

Phase 9의 대표 Codex profile 및 fullstack language variant dry-run은 모두 완료됐다. 이는 generator 구현이나 실제 설정 export 완료를 의미하지 않는다.
