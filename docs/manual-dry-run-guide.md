...
| `fullstack-spring-react` | `examples/codex/fullstack-spring-react/` | `ready` | Kotlin backend variant와 대응 Spring bridge, fullstack API 경계 |

각 위치의 README, preview, merge trace, skill selection과 validation report는 수동 검증 example이며 실제 export artifact가 아니다.

`fullstack-spring-react` dry-run은 Kotlin option과 Kotlin–Spring bridge를 선택해 required variant를 해소했다. Java option은 profile의 alternative로 유지하며 필요하면 별도 dry-run으로 검증한다.
