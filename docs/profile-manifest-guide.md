...
현재 registry catalog와 generator가 없으므로 `profile.yml`은 저장소 상대 경로를 참조하는 수동 검토용 초안이다. Registry 식별자와 schema가 확정되면 경로 참조를 catalog ID로 전환하되 논리적 조합 의미는 유지한다.
...
현재는 6개 profile manifest 초안과 수동 검토 규칙만 정의한다. 실제 profile catalog, compatibility matrix, generator, adapter, installer와 Web UI는 만들지 않는다.
