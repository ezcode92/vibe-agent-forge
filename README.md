...
Phase 완료는 해당 문서·설계 자산 범위의 완료를 뜻한다. Fragment, skill과 profile의 catalog 상태는 여전히 `draft`다. Phase 10-3에서는 Codex `ready` dry-run coverage가 있는 조합을 `reviewed-for-mvp`, coverage가 없는 유효 조합을 `ready-candidate`로 판정하도록 확정했지만 catalog YAML을 승격하거나 실제 generator 구현을 시작하지 않았다.
