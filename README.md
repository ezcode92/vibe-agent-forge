...
| Phase 8 | 진행 중 | MVP generator 설계 문서는 존재하며 구현 전 결정사항이 남아 있음 |
| Phase 9 | 완료 | Codex manual dry-run 6종 모두 `ready` |
| Phase 10-1 | 완료 | 문서 상태, Phase와 lifecycle 설명 정합화 |

Phase 완료는 해당 문서·설계 자산 범위의 완료를 뜻한다. Fragment, skill과 profile의 catalog 상태는 여전히 `draft`이며 실제 실행 기능의 안정 상태를 뜻하지 않는다. Phase 8의 남은 판단은 `docs/generator-pre-implementation-decisions.md`에서 관리한다.
