...
| Phase 10-3 | 완료 | Catalog lifecycle 승격 기준과 MVP readiness 정책 확정 |
| Phase 10-4 | 완료 | Read-only MVP 구현 진입 계획과 파일 구조 계약 확정 |
| Phase 11-0 | 완료 | Python runtime, dependency와 Phase 11-1 구현 승인 계약 확정 |

Phase 완료는 해당 문서·설계 자산 범위의 완료를 뜻한다. Fragment, skill과 profile의 catalog 상태는 여전히 `draft`다. Phase 11-0에서 Python `>=3.12`, `uv`, `PyYAML`, `pytest`·`ruff`·`mypy`와 Phase 11-1 파일 범위를 확정했지만 초기화 파일이나 실제 generator/validator 구현은 만들지 않았다.
