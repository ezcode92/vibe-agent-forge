...
Phase 9에서는 `backend-kotlin-spring-rdb`, `python-cli-automation`, `frontend-react`, `flutter-app`, `fullstack-spring-react` profile과 Codex adapter를 대상으로 문서 기반 manual dry-run을 수행해 대표 조합의 preview 가능성을 검증합니다. Fullstack은 필수 Java/Kotlin 선택 전 `blocked`로 판정했으며, 모든 dry-run example은 실제 agent 설정으로 export하지 않습니다.
