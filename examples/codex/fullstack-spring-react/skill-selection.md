...
20개 skill의 path와 hard dependency는 모두 유효하다. Backend language variant 미선택은 skill 집합을 바꾸지 않지만 전체 preview readiness는 별도 validation error로 차단된다. 실제 skill 설치와 실행은 수행하지 않았다.
