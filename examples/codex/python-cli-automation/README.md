...
Generator 구현 없이 `python-cli-automation` profile의 fragment·skill 구성이 Codex project instruction preview로 조합 가능한지 검증한다. Python 언어 규칙, automation skill routing, architecture pending과 미확정 project metadata가 Codex adapter 계약에 따라 구분되는지 확인한다.
...
| `merge-trace.md` | Fragment 순서, dependency, pending과 중복 기록 |
| `skill-selection.md` | Common·testing·automation skill routing 기록 |
| `validation-report.md` | 수동 검증과 readiness 판정 |

## Pending과 제약

- CLI 및 automation 목적에 맞는 architecture fragment가 아직 선택되지 않았고 profile에 `architecture-fragment` pending으로 명시돼 있다.
