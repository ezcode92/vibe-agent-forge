# DRY-RUN PREVIEW: Java Spring React Fullstack 작업 지침

> **Preview example — 실제 `AGENTS.md`가 아닙니다.** `fullstack-spring-react` profile의 Java variant와 Codex adapter를 수동 조합한 문서이며 그대로 복사·적용·export하지 않습니다.

## Project Overview (프로젝트 개요)

이 preview는 Java Spring backend, JavaScript React frontend, RDB, RESTful API와 Modular Monolith를 사용하는 가상의 fullstack 프로젝트를 전제로 한다. 실제 project 목적, 구조, version과 운영 제약은 입력되지 않았다.

## Tech Stack (기술 스택)

- Backend: Java, Spring, RDB, Modular Monolith
- Backend language variant: `java` 선택됨
...
- Required language bridge: Java–Spring
...
- Backend: `stacks/language/java/agents.fragment.md`, `stacks/framework/spring/agents.fragment.md`, `stacks/database/rdb/agents.fragment.md`, `stacks/architecture/modular-monolith/agents.fragment.md`
...
- Bridge: `stacks/bridge/java-spring/agents.fragment.md`, `stacks/bridge/spring-rdb/agents.fragment.md`, `stacks/bridge/spring-modular-monolith/agents.fragment.md`, `stacks/bridge/javascript-react/agents.fragment.md`, `stacks/bridge/react-spring-api/agents.fragment.md`

Resolved fragment는 공통 13개와 Java option 2개를 합한 15개다. Kotlin option은 이번 preview에 포함하지 않으며 fragment 전문도 inline하지 않는다.
...
### Java Backend와 Module

- Java 객체의 불변식, null 허용, exception 계약과 collection 소유권을 API 경계에 명시한다.
- Java object와 Spring component 책임을 일치시키고 record/sealed type은 project Java version과 binding·proxy 요구를 확인한 뒤 사용한다.
...
- 이 preview는 Java fragment와 Java–Spring bridge만 적용한다.
- Kotlin fragment와 Kotlin–Spring bridge를 동시에 포함하지 않는다.
- Kotlin alternative는 `examples/codex/fullstack-spring-react/`의 별도 dry-run으로 유지한다.
...
- Java language fragment와 Java–Spring bridge가 정확히 하나의 variant 묶음으로 적용된다.
...
- Alternative: Kotlin backend는 별도 dry-run에서 검증됐으며 이번 preview에는 포함하지 않음
- Automation: Git-auto Stop hook을 사용하면 직접 commit·push하지 않고 `.gitauto/`를 staging하지 않는다. 기존 `.codex/hooks`는 명시 요청 없이 수정하지 않는다.
- Preview limitation: 이 문서는 Java variant 기준 dry-run example이며 실제 project instruction이나 export 결과가 아니다.
