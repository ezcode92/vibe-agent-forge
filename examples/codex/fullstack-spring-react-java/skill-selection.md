...
> Profile에 포함된 skill과 preview 노출 방식을 기록한 Java variant dry-run example이다. Skill 설치나 실행 결과가 아니다.
...
Profile skill 목록은 backend language option 밖에 선언돼 Java와 Kotlin variant에서 동일하다. Java 선택으로 language-specific skill을 추가하지 않으며 공통, backend, database, frontend, testing과 automation routing을 그대로 유지한다.
...
모든 skill 본문을 inline하면 서로 다른 작업 책임의 절차가 상시 context를 점유하고 fragment 규칙과 중복된다. 따라서 ID, 역할과 trigger만 노출하고 선택한 `SKILL.md` 본문만 lazy load한다.
...
20개 skill의 path와 hard dependency는 모두 유효하다. Skill 본문을 inline하지 않았으며 실제 skill 설치와 실행도 수행하지 않았다.
